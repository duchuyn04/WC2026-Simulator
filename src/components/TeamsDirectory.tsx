    ...
    <section className="mx-auto max-w-7xl px-4 py-8">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {filteredTeams.map((team) => (
          ...
          >
            <div className="relative border-b border-zinc-800 bg-[radial-gradient(circle_at_top_left,rgba(245,158,11,0.18),transparent_16rem)] p-5">
              <div className="flex items-start justify-between gap-4">
                <img
                  src={flagCdnUrl(team.code, 160)}
                  alt={`Cờ ${team.name}`}
                  className="h-16 w-24 rounded-2xl object-cover ring-1 ring-white/25"
                  onError={(event) => {
                    event.currentTarget.src = flagUrl(team.code);
                  }}
                />
                <div className="flex flex-col items-end gap-2 text-right">
                  <span className="rounded-full bg-zinc-950/80 px-3 py-1 text-xs font-black text-amber-300">
              ...
              <h3 className="mt-5 text-2xl font-black leading-tight tracking-tight group-hover:text-amber-300">
                {team.name}
              </h3>
            ...
            <div className="grid grid-cols-3 divide-x divide-zinc-800 text-center text-xs text-zinc-500">
              <div className="p-3">
                <b className="block text-base text-zinc-100">{team.squadCount}</b>
                <span className="text-zinc-300">Cầu thủ</span>
              </div>
              <div className="p-3">
                <b className="block text-base text-zinc-100">{team.appearances ?? "-"}</b>
                <span className="text-zinc-300">Lần dự</span>
              </div>
              <div className="p-3">
                <b className="block truncate text-base text-zinc-100">{team.headCoachName ?? "-"}</b>
                <span className="text-zinc-300">HLV</span>
              </div>
            </div>
          </Link>
        ))}
